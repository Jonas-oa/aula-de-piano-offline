...
// Fluxo assíncrono: envia o PDF, acompanha o job e baixa o MusicXML pronto.
...
export async function convertViaService({
  ...
  onProgress("Enviando o PDF ao serviço…");
  let response;
  try {
    response = await fetch(`${base}/v1/jobs`, { method: "POST", body: form });
  } catch (error) {
  ...
  while (!["completed", "failed"].includes(job.status)) {
    ...
    await delay(pollMs);
    const poll = await fetch(`${base}/v1/jobs/${job.id}`);
